from PathPlanning.RRTConnect import RRTConnectPlanner
...
def main():
    ...
    rrtPlanner = RRTConnectPlanner(map, epsilon=0.05, stepSize=10)
    while map.is_open:
        ...
        for node in rrtPlanner.nodeList1:
            # map.add_geometry(type='point', pos=node.pos.tuple())
            if node.parent is not None:
                map.add_geometry(type='line', start=node.parent.pos.tuple(), end=node.pos.tuple(), color=(100, 0, 0))
        for node in rrtPlanner.nodeList2:
            # map.add_geometry(type='point', pos=node.pos.tuple())
            if node.parent is not None:
                map.add_geometry(type='line', start=node.parent.pos.tuple(), end=node.pos.tuple(), color=(0, 0, 100))
